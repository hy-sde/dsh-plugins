/**
 * Default adapter table shipped with the DAP seam: name → adapter launch
 * configuration. A workspace `dap.json`/`dap.yaml` (or a user-level one) may
 * override any entry or add new ones — see `config.ts`. Ported from
 * oh-my-pi's `coding-agent/src/dap/defaults.json` (MIT) with the
 * js-debug adapter wired for the DSH js-debug binary lookup.
 * @module @hy-sde-org/dsh-dap/defaults
 */

import type { DapAdapterConfig } from './types.ts'

/** Default adapter table: adapter name → launch configuration. */
export const DEFAULT_ADAPTERS: Record<string, DapAdapterConfig> = {
  gdb: {
    command: 'gdb',
    args: ['-i', 'dap'],
