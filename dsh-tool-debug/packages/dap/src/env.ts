/**
 * Non-interactive environment defaults for debug adapter and debuggee child
 * processes: pagers, editors, and interactive prompts must never block a
 * headless debug run. Ported from oh-my-pi's
 * `coding-agent/src/exec/non-interactive-env.ts` (MIT).
 * @module @hy-sde-org/dsh-dap/env
 */

/** Portable command that rejects credential prompts without assuming an FHS layout. */
const REJECT_PROMPT_COMMAND = 'false'

/** Environment overrides that keep debug adapter/debuggee child processes non-interactive. */
