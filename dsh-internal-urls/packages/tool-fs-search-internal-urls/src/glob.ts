 ...
 * @module @hy-sde-org/dsh-tool-fs-search-internal-urls/glob
 */
...
export function applyGlobTool(ctx: Context, caps: GlobToolCaps): void {
  ...
  ctx.systemPrompt.section({
    ...
    text: ({ scope }) => ctx.tools.get('glob', scope) === undefined ? '' : ('Use the glob tool — not shell find — to discover files by path pattern. A pattern with no "/" matches basenames at any depth, so "*" matches every file in the tree rather than its top level. '
