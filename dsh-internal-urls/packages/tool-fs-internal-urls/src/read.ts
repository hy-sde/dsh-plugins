 ...
 * @module @hy-sde-org/dsh-tool-fs-internal-urls/src/read
 */
...
export function applyReadTool(ctx: Context, caps: ReadToolCaps): void {
  ctx.systemPrompt.section({
    ...
    text: ({ scope }) => ctx.tools.get('read', scope) === undefined ? '' : ('Use the read tool — not shell commands like cat — to inspect text files. Results include line numbers. Use offset and limit to continue reading large files.'),
  })
