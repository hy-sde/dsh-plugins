 ...
 * @module @hy-sde-org/dsh-tool-fs-internal-urls/src/write
 */
...
export function applyWriteTool(ctx: Context, sandbox: FsSandboxController): void {
  ctx.systemPrompt.section({
    ...
    text: ({ scope }) => ctx.tools.get('write', scope) === undefined ? '' : ('Use the write tool to create files or completely replace file contents. Existing files are overwritten, so read an existing file first (the default fs-observation-policy requires it) and prefer edit for targeted changes.'),
  })
