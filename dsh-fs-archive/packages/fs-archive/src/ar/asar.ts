...
import { readUInt32LE, writeUInt32LE } from './bytes.ts'
import { ArchiveError } from './error.ts'
import { assertArchiveMemberSize, assertEntryCount, assertIndexSize } from './limits.ts'
import {
...
class UnpackedAsarMemberSource implements MemberSource {
  ...
  async read(size: number, memberPath: string): Promise<Uint8Array> {
    ...
    const stat = await fs.stat(this.#filePath).catch(() => {
    ...
    let bytes: Uint8Array
    try {
      bytes = new Uint8Array(await fs.readFile(this.#filePath))
    } catch (error) {
