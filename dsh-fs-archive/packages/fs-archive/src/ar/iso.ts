...
import { bytesMatchAscii, readUInt16BE, readUInt16LE, readUInt32BE, readUInt32LE } from './bytes.ts'
import { upsertArchiveEntry } from './entries.ts'
import { ArchiveError } from './error.ts'
import { type ArchiveLimits, assertArchiveMemberSize, assertEntryCount, assertIndexSize } from './limits.ts'
import { assertArchivePathBytes, normalizeArchiveEntryPath, normalizeArchiveLookupPath } from './paths.ts'
import type { ByteSource } from './source.ts'
import type { ArchiveIndexEntry, FormatReader, MemberSource } from './types.ts'
...
async function readVolume(source: ByteSource, budget: MetadataBudget, limits: ArchiveLimits): Promise<IsoVolume> {
  ...
  if (primary === undefined) throw invalidIso('primary volume descriptor not found')
  return primary
}
