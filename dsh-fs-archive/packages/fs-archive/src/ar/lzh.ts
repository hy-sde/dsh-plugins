import { crc16Arc } from './checksums.ts'
import { ArchiveError } from './error.ts'
import { assertArchiveMemberSize, assertEntryCount, assertIndexSize, assertInMemorySize } from './limits.ts'
import { assertArchivePathBytes, assertArchivePathString, normalizeArchiveEntryPath } from './paths.ts'
import { type ByteSource, readAllBytes } from './source.ts'
import type { ArchiveIndexEntry, FormatReader, FormatReadOptions, MemberSource } from './types.ts'
...
export function decompressLhStatic(
  ...
  while (outputPosition < outSize) {
    ...
    if (commands === undefined || positions === undefined) {
      throw new ArchiveError(`Invalid ${label} compressed data: missing Huffman trees`)
    }
    blockRemaining--
...
function parseLzhHeader(bytes: Uint8Array, offset: number, options: FormatReadOptions): ParsedLzhHeader {
  ...
  if (fields.commonCrc !== undefined) {
    const commonCrcOffset = fields.commonCrcOffset
    if (commonCrcOffset === undefined) throw new ArchiveError('Invalid LZH common header CRC-16 offset')
    const relative = commonCrcOffset - offset
