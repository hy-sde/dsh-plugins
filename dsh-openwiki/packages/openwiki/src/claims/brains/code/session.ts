...
import { applyClaimOperations, cloneClaims } from '../../core/mutations.ts'
import { cacheEvidenceResolver } from '../../core/resolver-cache.ts'
import type { Claim, EvidenceResolver } from '../../core/types.ts'
import { normalizeWikiPagePath } from './paths.ts'
  ...
  async finalize(
    ...
    for (const [page, state] of this.pages) {
      ...
      verificationByPage.set(
        page,
        eligible && state.verification !== undefined
          ? { ...state.verification }
          : null,
      )
    }
