/**
 * Child-scope registration and delivery suite for the ported report tool.
 ...
 * — MIT, see THIRD-PARTY-NOTICES.md. Every assertion is preserved.
 *
 * Testkit adaptation: the published `@deepseek-ai/dsh-subagent@0.1.2-rc.1`
 * predates the fork's report channel, so instead of the fork's
 * `SubagentRuntime` + `SubagentSpawn` the suite mounts the local seam
 * (`./report-subagent-seam.ts`), which implements the fork contract over the
 * real published agent-loop/agents/sessions/tools/system-prompt stack. All
 * child materialization, inbox ordering, waking, and settlement remain the
 * published machinery's.
 */
...
import { subagentsReport } from '../src/report-contract.ts'
import * as SubagentSeam from './report-subagent-seam.ts'
...
/** Boot the real published continuation stack with optional report installation. */
async function setup(options: { load?: boolean; config?: tool.Config } = {}) {
  ...
  // The published dsh-subagent predates the fork report channel; mount the
  // local seam implementing the fork contract over the published stack.
  await ctx.plugin(SubagentSeam)
...
describe('dsh-tool-subagent-report', () => {
  ...
  it('normalizes a direct parent send rejection', async () => {
    ...
    await expect(subagentsReport(ctx).reportFrom(child, [{ type: 'text', text: 'rejected' }], {
      delivery: 'quiet',
      signal: testSignal,
  ...
  it('rejects roots, forged same-id senders, absent parents, cancellation, and drain', async () => {
    const { ctx, parent, adapter } = await setup()
    await expect(subagentsReport(ctx).reportFrom(parent, [{ type: 'text', text: 'root' }], {
      delivery: 'quiet',
      signal: testSignal,
    ...
    await expect(subagentsReport(ctx).reportFrom(forged, [{ type: 'text', text: 'forged' }], {
      delivery: 'quiet',
      signal: testSignal,
    })).rejects.toMatchObject({ code: 'UNAUTHORIZED' })
    ...
    await expect(subagentsReport(ctx).reportFrom(child, [{ type: 'text', text: 'draining' }], {
      delivery: 'quiet',
      signal: testSignal,
  ...
  it('rolls back materialization when a setup contribution revokes itself', async () => {
    ...
    self.revoke = subagentsReport(ctx).registerContinuableSetup((childCtx) => {
  ...
  it('rolls back materialization when setup revocation lands before publication', async () => {
    ...
    const self: { revoke?: () => void } = {}
    let installed = false
    self.revoke = subagentsReport(ctx).registerContinuableSetup(() => {
      installed = true
...
describe('dsh-tool-subagent-report keyed decisions', () => {
  it('records a keyed needs-decision report as an open decision for the parent', async () => {
    ...
    const decisions = subagentsReport(ctx).listOpenDecisions(parent)
    ...
    expect(subagentsReport(ctx).listOpenDecisions(parent)).toHaveLength(1)

    // The parent can close the record with the same key.
    expect(subagentsReport(ctx).resolveOpenDecision(parent, child.id, 'approach-choice')).toBe(true)
    expect(subagentsReport(ctx).listOpenDecisions(parent)).toEqual([])
  })
  ...
  it('records a blocked report with its blocker and keeps it after the child settles', async () => {
    ...
    expect(subagentsReport(ctx).listOpenDecisions(parent)[0]).toMatchObject({
