...
| [**Agent Graph — durable scheduling**](dsh-graph/README.md) | `dsh-graph-control` · `dsh-graph-stream` · `dsh-graph-executor` · `dsh-tool-graph` · `dsh-graph-wakes` · `dsh-graph-projection` · `dsh-graph-host` | The main agent becomes the supervisor of a durable work schedule: `view/update/yield_agent_graph` add, stop and finish work with deterministic ids and exactly-once claims (CAS control store); children are executable operators with worktree leases; the host wakes the supervisor at durable checkpoints — no polling. The schedule authority for the orchestration plane above. | [Apache Maka](https://github.com/apache/maka) (No. 1) |
...
| [**Debugging**](dsh-tool-debug/README.md) | `dsh-dap` (engine) · `dsh-tool-debug` (model) | One `debug` tool, 28 operations: launch/attach real debuggers (debugpy, lldb-dap, gdb, dlv, …), source/function/instruction/data breakpoints, continue/step/pause, threads, stack traces, scopes/variables/evaluate, memory read/write. | [oh-my-pi](https://github.com/can1357/oh-my-pi) `coding-agent/src/dap/*` + `tools/debug.ts` + DSH |
...
| [**Secret / hardening audit**](dsh-av/README.md) | `dsh-av` (engine) · `dsh-tool-av` (model) | Read-only vault audit: scan for exposed credentials, check hardening, inspect the detector/hardener catalog, list secret *names* — the tool never returns a secret value. | fork `@deepseek-ai/dsh-av` (macOS `av` CLI) |
...
| **Structural code tools** | `dsh-tool-ast` | `ast_grep` structural search + `ast_edit` structural rewrite over the packaged native ast-grep engine (AST-aware, not text). | [oh-my-pi](https://github.com/can1357/oh-my-pi) + ast-grep |
...
| **Library search** | `dsh-tool-library-search` | Free "has this already been built?" across npm/crates.io/Maven/Go/PyPI/RubyGems + GitHub — exact-name ranked, stars/downloads as fitness signals, no API key. | DSH |
| **Session health** | `dsh-session-intelligence` | `session_health`: how did past sessions actually go — outcome classification (with confidence), A–F grading with per-signal penalties, tool-health and context-pressure signals. | agentsview (MIT, Kenn Software) + DSH |
...
| **Session URL scheme** | `dsh-session-url` | `session://` handler: your own session history becomes files — list sessions, rendered transcripts, exact event JSON, FTS cross-session search via internal-URL-aware read/grep. Optional companion to internal-URL routing. | oh-my-pi internal-urls + DSH |
| **Credential-free web search** | `dsh-web-search-public` | One `web_search` call fans out to Startpage/DuckDuckGo/Ecosia/Google/Mojeek in parallel, merges by consensus, respects soft/hard deadlines — zero API keys. | [oh-my-pi](https://github.com/can1357/oh-my-pi) `web/search/*` + [Effect](https://effect.website/) |
