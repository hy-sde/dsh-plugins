...
- [`@hy-sde-org/dsh-graph-control`](../graph-control/README.md): the durable wake rows and the begin/complete CAS this runtime settles.
- [`@hy-sde-org/dsh-graph-stream`](../graph-stream/README.md): `AgentGraphCoordinator`, re-driven by the host deliver hook.
- [`@deepseek-ai/dsh-schedule`](https://github.com/deepseek-ai/deepseek-harness/tree/master/packages/schedule/schedule) — the observation pattern (`agent/status === 'idle'` + `agent.whenIdle`) this runtime mirrors.
- Maka reference: [`agent-graph-supervisor-wake.ts`](https://github.com/apache/maka/blob/main/packages/runtime/src/agent-graph-supervisor-wake.ts) (authoritative wake semantics).
- `tests/graph-wakes.spec.ts`: real sqlite-backed store with a fake idle observer and deliver hook.

## Model Experience

No model-facing surface. This package is host-side machinery; the supervisor tools in `dsh-tool-graph` are what the model sees, and the host wiring turns `deliver` into the model-visible checkpoint turn.
...
- The graph-level stop convention (log stop with `targetId` equal to the root/graph id) is defined here; a work-item stop never suppresses wakes. `dsh-tool-graph` must commit graph stops with the root identity for suppression to engage.
