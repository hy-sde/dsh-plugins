...
`dsh-graph-executor` 是 Agent Graph 的子操作员执行器适配器。它基于两条注入接缝实现协调器的 `AgentGraphExecutor` 表面：`GraphOperatorWorktreePool`（为同一仓库获取/释放工作树租约）与 `GraphOperatorChildRunner`（每个激活一次提供方支撑的子运行）。控制存储仍是持久化权威——预置与绑定都是行，其余一切皆为派生。
...
本包不提供任何工具、提示词或插件行——由流层协调器与主管工具消费。
...
控制存储新增一张权威表 `operator_bindings`（行键 `provisionId`），并带有一个在打开时重建的派生索引 `${graphId}:${workId}`。方法：`bindOperatorWorktree`（同租约幂等重绑保留原始 `boundAt`；异租约重绑抛 `binding-conflict`）、`readOperatorBinding`、`readOperatorBindingByWork`、`listOperatorBindings(graphId?)`。

<a id="further-exploration"></a>
## 进一步探索

- [`@hy-sde-org/dsh-graph-control`](../graph-control/README.zh.md)：存储行（预置、绑定）及其持久化契约。
- [`@hy-sde-org/dsh-graph-stream`](../graph-stream/README.zh.md)：`AgentGraphExecutor`、记录折叠（`readCommittedAgentGraphProjection`）、`stableHash32`、`truncateUtf8`。
- Maka 参考：[`git-worktree-child-executor.ts`](https://github.com/apache/maka/blob/main/packages/storage/src/git-worktree-child-executor.ts)（租约身份、确定性路径、工作树跨终结运行存活）与 [`session-manager.ts`](https://github.com/apache/maka/blob/main/packages/runtime/src/session-manager.ts)（`runClaimedAgentGraphIntent`）。
- `tests/graph-executor.spec.ts`：假池/运行器加真实 sqlite 存储。

<a id="model-experience"></a>
## 模型体验

无面向模型的表面。包是宿主侧机制：不渲染任何提示词（由流层负责），它记录的终结摘要由主管工具呈现。

<a id="known-limitations-and-deferred-work"></a>
## 已知限制与后续工作

- 租约幂等性为进程本地：真实池实现通过在重启后匹配 `listWorktrees` 采纳先前已租用的工作树；此处不从存储绑定重新推导租约（绑定是持久化提示，不是租约权威）。
- 工作树按设计在终结运行后存活（Maka 契约）；执行器绝不释放租约。池释放由主机装配在图拆除时接入。
- `recordSink` 失败作为执行失败传播——协调器上报并重试；目前没有崩溃一致的终结事件日志。
- `runClaimedAgentGraphIntent` 遵循 dsh-graph-stream README 记录的接缝契约（`provisionOperator` 可能返回 `undefined`）；`dsh-graph-stream` 的 `src/types.ts` 中的 `AgentGraphExecutor` 类型仍声明了非可选预置结果，期望在集成时对齐。
