...
A rate-limited all-failed aggregate (transport signature, retries exhausted) surfaces the
same `WEB_PROVIDER_ERROR` with `public search engines are rate limited (…); retry in about Ns`.
