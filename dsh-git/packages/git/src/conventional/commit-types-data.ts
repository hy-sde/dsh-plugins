/**
 * Conventional-commit type vocabulary (llm-git canonical classification).
 * Ported verbatim from omp's `commit/conventional/resources/commit_types.json`.
 * @module @hy-sde-org/dsh-git/conventional/commit-types-data
 */

/** One entry of the llm-git commit-type vocabulary resource. */
export interface CommitTypeEntry {
  name: string
  description: string
...
/** Shape of the verbatim `commit_types.json` resource. */
export interface CommitTypesResource {
  classifier_hint: string
  types: CommitTypeEntry[]
}

/** Verbatim llm-git commit-type table (22 canonical types + aliases). */
export const COMMIT_TYPES_RESOURCE: CommitTypesResource = {
  ...
  'types': [
    ...
    },
    {
      ...
      'examples': [
        'Reverted commit abc123 introducing the regression → revert',
      ],
