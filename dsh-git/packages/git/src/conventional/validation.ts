 ...
 * Direct port of omp's `commit/conventional/validation.ts` — the deterministic
 * rules (summary past-tense first word, summary length limits, body-item
 * present-tense/period checks, scope rules, and advisory type/file-stat
 * consistency) driven by the verbatim `validation_data.json` tables.
 * @module @hy-sde-org/dsh-git/conventional/validation
 */

import * as path from 'node:path'
import type { ConventionalCommit } from './types.ts'
import { COMMIT_TYPE_ORDER, isCommitType } from './commit-types.ts'
import type { ConventionalConfig } from './normalization.ts'
import { VALIDATION_RESOURCE } from './validation-data.ts'
...
const ED_BLOCKLIST = new Set(VALIDATION_RESOURCE.ed_blocklist.map(value => value.toLowerCase()))
const D_BLOCKLIST = new Set(VALIDATION_RESOURCE.d_blocklist.map(value => value.toLowerCase()))
const CODE_EXTENSIONS = new Set(VALIDATION_RESOURCE.code_extensions.map(value => value.toLowerCase()))
const DOC_EXTENSIONS = new Set(VALIDATION_RESOURCE.doc_extensions.map(value => value.toLowerCase()))
const FILLER_WORDS = VALIDATION_RESOURCE.filler_words.map(value => value.toLowerCase())
const META_PHRASES = VALIDATION_RESOURCE.meta_phrases.map(value => value.toLowerCase())
const BODY_PRESENT_TENSE = new Set(VALIDATION_RESOURCE.body_present_tense.map(value => value.toLowerCase()))

/**
 * Return the configured past-tense form for a present-tense verb.
 * @param present - the verb to look up (case-insensitive).
 * @returns the matching past-tense form, or `undefined` when unknown.
 */
 ...
 * @param summary - the summary string to repair.
 * @returns the repaired summary, or `null` when no known verb matches.
 */
 ...
 * @returns `[verbSegment, suffix]`, or `null` when the token starts non-alpha.
 */
 ...
 * @param token - a single whitespace-delimited token.
 * @returns the lowercase stem, or `null` for all-caps/non-alpha tokens.
 */
 ...
 * @param word - the word to test (case-insensitive).
 * @returns true for -ed/-d verbs (minus blocklists) or irregular past forms.
 */
 ...
 * @param token - the first token of a summary.
 * @returns true for direct past forms, repaired stems, or `re-rewritten` forms.
 */
 ...
 * @param summary - the summary line.
 * @param commitType - the commit type (for type/stat consistency checks).
 * @param stat - optional `git diff --stat`-style text for advisory checks.
 * @returns the validation report (blocking `errors`, advisory `warnings`).
 */
 ...
 * @param message - the conventional commit to validate.
 * @param config - the summary/scope limits to check against.
 * @param options - optional `stat` text and `projectNames` for advisory checks.
 * @returns the validation report (blocking `errors`, advisory `warnings`).
 */
 ...
 * @param message - the conventional commit to check.
 * @param stat - `git diff --stat`-style text describing changed files.
 * @returns the advisory-only validation report.
 */
...
function addIssue(
  ...
  const issue: ValidationIssue = value === undefined
    ? { severity, field, code, message }
    : { severity, field, code, message, value }
...
function validateSummary(
  ...
  const firstLineLength =
    ...
    (message.scope ? Buffer.byteLength(message.scope, 'utf8') + 2 : 0) +
    2 +
...
function typeScopeConsistency(
  ...
  if (commitType === 'docs') {
    ...
    if (!hasDocs) {
      addIssue(report, 'warning', 'type', 'docs_without_docs', "Commit type 'docs' but no documentation files changed")
    }
