/**
 * Zstandard frame primitives shared by the JSONL session persistence backend
 * and the project memory bank. Both store append-only, checksummed, per-batch
 * zstd frames; this package owns the structural scan, the one-shot compress /
 * decompress API, torn-frame recovery, and the interchangeable multi-frame
 * decoders, so consumers never reimplement frame layout or Node-private
 * decoder lifecycle.
 *
 * Backed entirely by `node:zlib` — no native module, works in-process.
 * @module @hy-sde-org/dsh-memory/zstd-frame
 */
...
export { NodePrivateZstdFrameDecoder } from './zstd-private-decoder.ts'
export { PublicZstdFrameDecoder } from './zstd-public-decoder.ts'
 ...
 * @param maxOutput - optional plaintext bound; oversized frames reject with the Node error.
 * @returns the frame plaintext.
 */
export async function decompressZstdFrame(input: Buffer, maxOutput?: number): Promise<Buffer> {
  if (maxOutput === undefined) return zstdDecompressAsync(input)
  return zstdDecompressAsync(input, { maxOutputLength: Math.max(1, maxOutput) })
}

/**
 * Synchronous one-frame decompression with an optional plaintext bound — for
 * sync-only readers (the `memory:project` prompt section reads bank files with
 * `readFileSync`). Keeps checksum validation identical to the async path.
 * @param input - one structurally complete Zstandard frame.
 * @param maxOutput - optional plaintext bound; oversized frames reject.
 * @returns the frame plaintext.
 */
export function decompressZstdFrameSync(input: Buffer, maxOutput?: number): Buffer {
  if (maxOutput === undefined) return zstdDecompressSync(input)
  return zstdDecompressSync(input, { maxOutputLength: Math.max(1, maxOutput) })
}
